...
while True:
    ...
    if(len(lmList)) != 0:
        ...
        if fingers[1] and fingers[2]==False:
            ...
            xp,yp = x1,y1
        # Inside your main loop, in the drawing section

        if fingers[1] and not fingers[2]:  # If drawing mode is enabled
            cv2.circle(img, (x1, y1), 15, drawColor, cv2.FILLED)
            print("Drawing Mode")
            ...
            for i in range(1, 11):
                interX = xp + (x1 - xp) * i / 10
                interY = yp + (y1 - yp) * i / 10
                cv2.line(img, (int(xp), int(yp)), (int(interX), int(interY)), drawColor, thickness)
                cv2.line(imgCanvas, (int(xp), int(yp)), (int(interX), int(interY)), drawColor, thickness)
    ...
    # Check for 'R' key press to clear the canvas
    key = cv2.waitKey(1)
